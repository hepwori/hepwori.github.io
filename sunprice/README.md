...
- city labels are placed with a greedy collision-avoidance pass (real SVG `getBBox()` measurement, checked against both other labels and other cities' dots) so labels don't overlap. Not every city gets a label in the densest clusters; all 54 are always in the table.
- **Boulder's estimate**: added after the initial 53-city pull, straight from the Zillow ZHVI CSV — not from the Current Results table the other 53 came from, so it never had a `sunshineDays` value to begin with (`null` in the data). No independent NOAA/climate-normal sunshine-hours station exists for Boulder specifically, so its hours figure (3,107) is Denver's — Boulder's nearest first-order climate station, ~25 miles away, same semi-arid Front Range climate and elevation. At least one other public source (weather-and-climate.com) makes the same substitution for Boulder, borrowing Denver's station data outright.
